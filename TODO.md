...
- **NVDA 播報**（出處：第 3 項「四項驗收」第 3 點）
  **缺**：一套螢幕閱讀器（Windows 上 NVDA 免費）。
  **怎麼驗**：掃一次，確認「掃描完成，總分 XX 分」與錯誤訊息**兩者都有**播報。
  **已有的旁證**：用 CDP 的 `Accessibility.getFullAXTree` 確認 `#sr-status` 與 `#sr-alert`
  以 `status`／`alert` 兩個 role 出現在無障礙樹上，三個 accordion 按鈕的可及名稱與
  `expanded` 狀態也正確。**這只證明 role 與狀態暴露正確，不證明 NVDA 會唸出來**
  ——live region 的播報時機正是第 3 項改動的核心，只有真的開螢幕閱讀器才測得到。

- **實際冷啟動秒數 —— 2026-08-10 已量到，22.6 秒**（出處：第 4 項「待你驗收」第 2 點）
  **有效的量法（第三次才成功，前兩次作廢）**：連續 **16 分鐘**不對線上發任何請求，
  然後送一次計時的 `GET /`。**判讀方式只有一種——回應慢才是有效的冷啟動數字**；
  回應快代表安靜視窗被外部流量打斷（這是公開網址，你自己開頁面、搜尋引擎爬蟲都算），
  那就是量測無效。同日前兩次分別是 `time_total` 0.297 秒與
  `time_starttransfer` 0.195 秒，**兩次都是實例醒著，一律作廢，不可以拿來當
  「冷啟動很快」的證據**。
  **第三次成功**：安靜 16 分鐘後 `GET /` 回 HTTP 200，
  `time_starttransfer` = **22.603 秒**。這是實測值，不是推算。
  **這個數字推翻了原本寫在程式碼註解裡的假設**：`client/public/js/main.js:11` 寫的
  冷啟動 30–60 秒是業界普遍回報值，本站實測明顯比它快。
  **但這是單次取樣**，Render free plan 的冷啟動本來就會因映像檔載入與區域負載而浮動，
  所以第二次取樣（改用真實的 `POST /api/analyze`，同時驗端點在冷實例上的行為）
  尚在進行；`REQUEST_TIMEOUT_MS` 要不要從 90000 往下調，等兩個樣本都到齊再決定。
  **仍未涵蓋**：第 4 項「待你驗收」第 1 點的**瀏覽器行為**——在休眠實例上實際掃一次，
  目視確認第 5 秒跳出喚醒提示、最終成功出報告。上面這次量測只送了 `GET /`，
  沒有走前端流程。**現在已經有可重複的取得休眠實例的方法（安靜 16 分鐘），
  所以這一點不再是做不到，只是還沒做。**
...
- **`TRUST_PROXY_HOPS` 的雙 IP 實測**（出處：「已完成」2026-08-10 那則）
  ...
  **怎麼驗**：用筆電對線上網址**在 60 秒內**連送 11 次掃描（固定視窗會重置，慢了測不準），
  ...
  設定錯誤，要把值往上加到 `2`（一次加一階）；手機正常出報告代表設定正確。
  **已有的**：Render 儀表板的 Environment 頁面顯示 `TRUST_PROXY_HOPS` 存在、值是 `1`
...
那條 socket 建立時已通過檢查，重用不構成新風險）。**經評估決定不做的項目見
「2026-08-10 檢視後刻意不做的三項」，不需要重複提案。**
...
不是遺漏**，承擔的風險與有界的回退成本寫在第 5 項裡。下載報表那則 `style-src-elem`
違規依三個選項的第 1 案「不處理」結案。**我驗證過的**：本機與線上都確認回應帶正式的
...
哪一則違規是刻意留著的、以及回退方式。**沒有動 `client/`，所以沒有更新 `?v=`、
沒有 `build:css`。**

**我驗證過的**：

- `server/` 的 `npm test` → `# pass 97 / # fail 0`（沒有任何測試斷言標頭名稱，
  所以這是回歸把關，不是這次改動的驗收）。
- 本機起 server（PORT=3151）：回應帶 `Content-Security-Policy`，
  且 `Report-Only` 出現次數為 **0**——兩個標頭沒有同時送出。
