...
（**這一項現在比先前重要**——強制模式下若 `gateway.umami.is` 還有哪裡不對，
統計會靜默歸零，請在 24 小時內看一次後台），以及「已完成」裡 `TRUST_PROXY_HOPS`
的雙 IP 實測。
...
**待你確認（24 小時內看一次就好）：Umami 後台有沒有新的訪問資料。**
這是本項唯一沒被涵蓋的驗收條件，我沒有後台帳號。它也正是強制模式下最容易靜默壞掉的
地方——`gateway.umami.is` 若還有哪裡不對，統計會歸零而 Console 與畫面都不會有徵兆。
真的歸零就先回退成 Report-Only，再從違規清單找漏掉的網域。
