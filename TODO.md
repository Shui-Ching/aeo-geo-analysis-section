...
**commit 還沒 push**，push 與「切成正式強制」都是對外變更，都在等你決定。
...
### 5. 加上 Content-Security-Policy（Report-Only 已完成，觀察已做完，`connect-src` 已補，只剩「切正式」待你決定）
...
- **✅ 這一行已經改完並 commit（`bbc926f`，2026-08-10）。** 同時改掉那則會誤導人的
  ...
  **還沒 push，所以線上跑的仍是舊政策**（Report-Only 之下，這只代表 Console 繼續記
  那兩筆 `connect-src` 違規，統計照常運作，沒有使用者影響）。
...
**第 4 步（切成正式強制）現在還不能做**，原本三個前提，現在剩兩個：

1. ~~`connect-src` 要先補上 `gateway.umami.is`~~ **已完成（`bbc926f`）**，但那個 commit
   還沒 push，所以要切正式的話，順序是「先 push 補丁、確認線上政策字串已更新，再切」，
   不能兩件事一起推——中間任何一刻政策裡少了 `gateway.umami.is` 又是強制模式，統計就斷。
