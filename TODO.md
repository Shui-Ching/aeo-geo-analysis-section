...
1. ~~`connect-src` 要先補上 `gateway.umami.is`~~ **已完成並已上線（`bbc926f`），此前提解除。**
   本項原本寫「那個 commit 還沒 push，要切正式得先 push 補丁再切，不能兩件事一起推」
   ——**該顧慮已不存在**：實測線上的 Report-Only 標頭已含 `https://gateway.umami.is`
   （詳見本檔「2026-08-10 收尾對帳」），所以切正式時政策字串本身已經是對的，
   不會出現「強制模式下少一個網域、統計斷掉」的空窗。
